...
const chainCoordinates: Coordinate[] = [
    [...startCoordinates],
    [...startCoordinates],
    [...startCoordinates],
    [...startCoordinates],
...
const TailVisitedPositions = new Set([chainCoordinates[9].join(',')]);
...
const moveNextKnotPosition = (nextKnotInChainPosition: Coordinate, currentPosition: Coordinate, chainIndex: number) => {
    ...
    if (Math.abs(xDistance) > 2) {
        throw new Error(`xDistance should never be greater than 2! ${chainIndex}`,)
    }
    if (Math.abs(yDistance) > 2) {
        throw new Error(`yDistance should never be greater than 2! ${chainIndex}`,)
    }
    ...
    if (sameY) {
        ...
        if (Math.abs(xDistance) === 1) {
            ...
            // console.log('xdistance is one should return', {chainIndex})
            return;
        }
        ...
        // console.log('tail moved', {currentPosition, chainIndex})
        return;
    }
    ...
    if (Math.abs(xDistance) === 2 && Math.abs(yDistance) === 2) {
        // console.log('Double diagonal!!', {currentPosition, nextKnotInChainPosition})
...
rawData.forEach(row => {
    ...
    [...Array(distance)].map(() => 0).forEach(() => {
        ...
        chainCoordinates.forEach((coordinates, index) => {
            if (index === 0) {
                moveHead(direction as Direction, coordinates);
                return;
            }
            moveNextKnotPosition(chainCoordinates[index-1], coordinates, index)

        })

        TailVisitedPositions.add(chainCoordinates[9].join(','));
