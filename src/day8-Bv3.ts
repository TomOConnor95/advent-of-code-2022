...
console.time("day8time3");

const calculateViewingDistanceToLeft = (rows: number[][]) => {
    // Maybe this pre-calcualtion is worth the initial performance cost in some cases, but doesn't seem to make much of a difference
    const biggestValueToLeft = rows.map((row) => row.reduce<number[]>((acc, curr, innerIndex) => {
        if (innerIndex === 0) {
            return [0];
        }
    ...
    return rows.map((row, outerIndex) => row.reduce<number[]>((acc, curr, innerIndex) => {
        if (innerIndex === 0) {
            return [0];
        }
        const prevValue = row[innerIndex -1];
        ...
        if (curr > biggestValueToLeft[outerIndex][innerIndex]) {
