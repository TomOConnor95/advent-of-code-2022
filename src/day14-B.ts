...
for (let x = minimumX - maximumY; x<=maximumX +maximumY; x++) {
    for (let y = minimumY -1; y<=maximumY +2; y++) {
        if (!dataGrid[y]) {
            dataGrid[y] = [];
        }
        if(y === maximumY +2) {
...
const simulateSandFalling = (data: DataGrid) => {
    ...
    while (isMoving && keepRunningSimulation) {
        ...
        numberOfSettledSand++;
        if ((sandPosition.distanceFromTop === sandEntryCoordinate.distanceFromTop) &&
            (sandPosition.distanceFromLeft === sandEntryCoordinate.distanceFromLeft)) {
